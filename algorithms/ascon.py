...
def ascon_hash(message, variant="Ascon-Hash", hashlength=32):
    ...
    # Initialization
    tagspec = int_to_bytes(
        256 if variant in ["Ascon-Hash", "Ascon-Hasha"] else 0, 4)
    S = bytes_to_state(
        to_bytes([0, rate * 8, a, a-b]) + tagspec + zero_bytes(32))
    if debug:
...
def ascon_mac(key, message, variant="Ascon-Mac", taglength=16):
    ...
    """
    assert variant in ["Ascon-Mac", "Ascon-Prf",
                       "Ascon-Maca", "Ascon-Prfa", "Ascon-PrfShort"]
    ...
    # bytes (input rate for Mac, Prf)
    msgblocksize = 40 if variant in ["Ascon-Prfa", "Ascon-Maca"] else 32
    rate = 16  # bytes (output rate)

    if variant == "Ascon-PrfShort":
        # Initialization + Message Processing (Absorbing)
        IV = to_bytes([len(key) * 8, len(message)*8, a +
                      64, taglength * 8]) + zero_bytes(4)
        ...
        # Finalization (Squeezing)
        T = int_to_bytes(S[3] ^ bytes_to_int(key[0:8]), 8) + \
        ...
        S = bytes_to_state(to_bytes(
            [len(key) * 8, rate * 8, a + 128, a-b]) + tagspec + key + zero_bytes(16))
        if debug:
            printstate(S, "initial value:")
        ...
        # Message Processing (Absorbing)
        m_padding = to_bytes(
            [0x80]) + zero_bytes(msgblocksize - (len(message) % msgblocksize) - 1)
        m_padded = message + m_padding
...
def ascon_initialize(S, k, rate, a, b, key, nonce):
    ...
    iv_zero_key_nonce = to_bytes(
        [k, rate * 8, a, b]) + zero_bytes(20-len(key)) + key + nonce
...
def ascon_process_associated_data(S, b, rate, associateddata):
    ...
    if len(associateddata) > 0:
        a_padding = to_bytes([0x80]) + zero_bytes(rate -
                                                  (len(associateddata) % rate) - 1)
